...
def crop_image(image, x1, y1, x2, y2, torso=False):
...
def draw_connections(frame, keypoints, edges, confidence_threshold, original_image):
    # y, x, c = frame.shape
    # shaped = np.squeeze(np.multiply(keypoints, [y, x, 1]))
    left_shoulder = None
    right_shoulder = None
    ...
    if left_shoulder and left_hip and right_shoulder and right_hip is not None:
        ...
        crop_image(original_image, left_shoulder, left_hip, right_shoulder, right_hip, torso=True)
...
# ran_img = cv2.imread("./imdb-images/04/" + il[random.choice(range(0, len(il)))])
# ran_img = cv2.cvtColor(ran_img, cv2.COLOR_BGR2RGB)
# il = os.listdir("women_pics")
# il = np.array(il)
# ran_img = cv2.imread("./women_pics/" + il[random.choice(range(0, len(il)))])
# ran_img = cv2.cvtColor(ran_img, cv2.COLOR_BGR2RGB)

# il = os.listdir("man_pics")
# il = np.array(il)
# ran_img = cv2.imread("./man_pics/" + il[random.choice(range(0, len(il)))])
# ran_img = cv2.cvtColor(ran_img, cv2.COLOR_BGR2RGB)
#
#
# # Resize and pad the image to keep the aspect ratio and fit the expected size.
...
#
# #  RENDERING
# draw_connections(ran_img, keypoints_with_scores, EDGES, 0.4)
# draw_keypoints(ran_img, keypoints_with_scores, 0.4)
# # ran_img = cv2.resize(ran_img, (400, 400))
# # cv2.imshow('MoveNet Lightning Singlepose', cv2.cvtColor(ran_img, cv2.COLOR_BGR2RGB))
# # cv2.waitKey(0)
# plt.imshow(ran_img)
# plt.show()
...
# draw_connections(img_resized, keypoints_for_resized, EDGES, 0.4)
# draw_keypoints(img_resized, keypoints_for_resized, 0.4)
# plt.imshow(img_resized)
# plt.show()

# draw_connections(img_resized, keypoints_with_scores, EDGES, 0.4)
# draw_keypoints(img_resized, keypoints_with_scores, 0.4)
# plt.imshow(img_resized)
# plt.show()
...
draw_connections(frame, keypoints_with_scores, EDGES, .4, original_image)
draw_keypoints(frame, keypoints_with_scores, 0.4)
...
#
# # Rendering
# draw_connections(frame, keypoints_with_scores, EDGES, .4)
# draw_keypoints(frame, keypoints_with_scores, 0.4)
# # # cv2.imwrite("image_with_keypoints_original.png", frame)
# plt.imshow(frame)
# plt.show()